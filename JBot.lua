...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderate servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.5.8. Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>admins' then
		message.channel:sendMessage(string.format(' ``` Admin List: 189513922808774656/Katyusha 178525861476040704/Unique, 188832914639814656/JBot, 182501713423499264/Kongou, 179238629829246976/Obunaga ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>cmds' then
		message.channel:sendMessage(string.format(' ``` Comamnds: @Creator: >test, >servers, >stats, >disconnect, >todo, >setgame, >setidle, >setonline @Admin: >punish <@user> >kick <@user>, >ban <@user>, >warn <@user> >admins, >creator, >clearchat @Free: >cmds, >joke, >invite, >help, >about, >home, >new ```, %s', message.author:getMentionString()))
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.5.8: No longer can kick JBot with >kick <@JBot#7214>, added >punish for @Admins ```, %s', message.author:getMentionString()))
